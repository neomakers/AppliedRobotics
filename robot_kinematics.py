...
def plot_robot_forward(theta1, theta2):
    ...
    if y1 >= 0:
        ...
        ax.text(x1, 0 + delta * 2, r'$P_{1_x}=l_1\cos(\theta_1)=%.2f$' % x1, fontsize=10, ha='center', color='red')
    if y2 >= 0:
        ax.text(x2, 0 - delta * 5, r'$x=P_{2_x}=%.2f$' % x2, fontsize=10, ha='center', color='blue')
    else:
        ax.text(x2, 0 + delta * 5, r'$x=P_{2_x}=%.2f$' % x2, fontsize=10, ha='center', color='blue')
    ...
    if x2 >= 0 and y2 > 0:
        ax.text(0 - delta * 16, y2, r'$y=P_{2_y}=%.2f$' % y2, fontsize=10, va='center', color='blue')
    elif y2 < 0:
        ax.text(0 - delta * 18, y2, r'$y=P_{2_y}=%.2f$' % y2, fontsize=10, va='center', color='blue')
    else:
        ax.text(0 + delta * 0.5, y2, r'$y=P_{2_y}=%.2f$' % y2, fontsize=10, va='center', color='blue')
